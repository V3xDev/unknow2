...
:: Criar tarefa com configurações que PREVINEM loops
...
if %errorLevel% equ 0 (
    ...
    schtasks /query /tn "%TAREFA%" /fo LIST /v | findstr /i "TaskName\|Status\|Next\|Task To Run"
) else (
    echo [AVISO] Tarefa nao encontrada, tentando metodo alternativo...
    echo.
    schtasks /create /tn "%TAREFA%" /tr "\"%CAMINHO%\"" /sc onlogon /ru SYSTEM /rl HIGHEST /delay 0030 /f >nul 2>&1
    if %errorLevel% equ 0 (
        ...
        if %errorLevel% equ 0 (
            ...
            echo [AVISO] Tarefa criada mas nao encontrada. Execute CRIAR_TAREFA.bat manualmente.
        )
    ) else (
        echo [ERRO] Falha ao criar tarefa agendada!
        echo.
        echo [INFO] Execute CRIAR_TAREFA.bat manualmente para criar a tarefa.
    )
)
