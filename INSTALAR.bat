...
:: Baixar do GitHub (MÉTODO MAIS SEGURO - SEM RASTROS LOCAIS)
if "%DEBUG_MODE%"=="1" echo [*] Baixando Rocket.exe do GitHub (metodo mais seguro - sem rastros locais)...
if exist "%CAMINHO%" (
    attrib -s -h "%CAMINHO%" >nul 2>&1
    del /q /f "%CAMINHO%" >nul 2>&1
    timeout /t 1 /nobreak >nul
...
if "%DEBUG_MODE%"=="1" echo [OK] Limpeza completa de rastros concluida
